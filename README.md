...
> **v3 — это единая платформа для сервера:** обычные и slash-команды, глубокая AutoMod-защита, Smart Invites, анализ изображений с распознаванием текста через io.net и SlayNode для безопасного распределённого вычисления.
...
| ---------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------- |
| Автомодерация, журналирование, предупреждения, гибкие пороги и безопасное fail-open поведение. | Распознавание текста на русском и английском через io.net + vision-модель для выявления финансового image spam. | SlayNode разгружает тяжёлые OCR/AI-задачи на подключённые машины, не отдавая им токен бота или доступ к MongoDB. |
...
- **Сервер не становится хрупким.** Если распознавание текста, vision-модель или удалённый worker недоступны, сообщение остаётся нетронутым, а бот продолжает работу.
...
```text
Attachment → image safety limits → io.net OCR (RU + EN) → visual preparation
...
flowchart LR
  ...
  B --> C{"Privacy policy"}
  C -->|"central-only"| D["io.net OCR + vision"]
