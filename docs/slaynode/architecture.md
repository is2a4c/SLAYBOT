...
Set `SLAYNODE.enabled` in `config.js`, set a random `SLAYNODE_MASTER_KEY` of at least 32 characters, and expose the control port through an HTTPS reverse proxy. The service provides `/health`, `/ready`, and Prometheus text at `/metrics`. Use `/slaynode enroll` with Manage Server permission, then run `SLAYNODE_ENROLLMENT_TOKEN=... SLAYNODE_CONTROL_URL=https://... npm run slaynode:enroll`. Store the returned ID and secret in the worker environment and start `docker compose -f docker-compose.slaynode.yml up -d`.

Worker containers are non-root, read-only, capability-free, PID/CPU/RAM limited, and each executor runs in a child process with a memory ceiling and timeout. Configure host-level GPU exposure separately; no shell command is accepted from a job.

Relevant environment variables:

- Control: `SLAYNODE_MASTER_KEY` (required when enabled).
- Worker identity: `SLAYNODE_CONTROL_URL`, `SLAYNODE_ID`, `SLAYNODE_SECRET`.
- Limits: `SLAYNODE_PARALLELISM`, `SLAYNODE_RAM_MB`, `SLAYNODE_JOB_TIMEOUT_MS`, `SLAYNODE_CPU_LIMIT`, `SLAYNODE_MEMORY_LIMIT`.
